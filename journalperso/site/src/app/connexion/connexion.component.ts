...
export class ConnexionComponent implements OnInit {
  ...
  constructor(private auth: AuthService, private activatedRoute: ActivatedRoute, private http:HttpClient, private router: Router, private userCom:UserCommunication) {}
  ...
  connexion(user:any){
    for (let userFor of this.utilisateurs){
      if(userFor.email === user.email && userFor.motdepasse === user.motdepasse){
        this.auth.login(user.email, user.motdepasse);
